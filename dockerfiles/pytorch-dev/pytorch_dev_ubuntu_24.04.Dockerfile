# Multi-stage build which download rocm binaries and then builds
# pytorch, pytorch vision and pytorch audio and finally produces
# a docker image with the built pytorch installed.
# This dockerfile requires docker buildkit to be installed.
#
# Build with:
...
COPY dockerfiles/pytorch-dev/docker_image_scripts/install_rocm_from_release.sh /
