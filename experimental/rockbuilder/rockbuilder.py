    ...
    if rocm_home_root_path.exists():
        ...
        if rocm_home_bin_path.exists():
            if rocm_home_lib_path.exists():
                if not is_directory_in_path_env_variable(
                    "PATH", str(rocm_home_bin_path)
                ):
                    # print("Adding " + str(rocm_home_bin_path) + " to PATH")
                    os.environ["PATH"] = (
                        str(rocm_home_bin_path)
                        + os.pathsep
                        + os.environ.get("PATH", "")
                    )
                if not is_directory_in_path_env_variable(
                    ENV_VARIABLE_NAME__LIB, str(rocm_home_lib_path)
                ):
        ...
        print(
            "Error, ROCM_HOME was not defined and ROCM build could not be found from the expected location: "
            + rocm_path
        )
        sys.exit(1)
...
parser.add_argument(
    "--configure",
    action="store_true",
    help="configure project for build",
    default=False,
)
