        ...
        if rocm_home_root_path.exists():
            ...
            if rocm_home_bin_path.exists():
                if rocm_home_lib_path.exists():
                    if not is_directory_in_env_variable_path("PATH", rocm_home_bin_path.as_posix()):
                    ...
                    if not is_directory_in_env_variable_path("PATH", rocm_home_llvm_path.as_posix()):
                        #print("Adding " + rocm_home_bin_path.as_posix() + " to PATH")
                        os.environ["PATH"] = rocm_home_llvm_path.as_posix() + os.pathsep + os.environ.get("PATH", "")
                    if not is_directory_in_env_variable_path(ENV_VARIABLE_NAME__LIB, rocm_home_lib_path.as_posix()):
